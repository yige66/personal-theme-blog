    ...
    <main className="article-page" style={{ '--theme': data.site.themeColor, '--accent': data.site.accentColor } as React.CSSProperties}>
      ...
      <section className="article-shell console-shell">
        ...
        <p className="article-summary">
          站点以前台可部署为第一目标：内容进入 data/blog.json，GitHub 保存版本，Vercel 负责预览与生产构建。本地写作后台保留为编辑辅助，不作为线上访客的操作入口。
        </p>
        <div className="console-steps">
          <span>1. 用 npm run cms 或直接编辑 data/blog.json 维护文章、动态、音乐、照片墙和友链。</span>
          <span>2. 执行 npm run check，确认测试、构建和内容数据都能通过。</span>
          <span>3. 提交 data、public/assets 和源码变更，让每次发布都有可回滚版本。</span>
          <span>4. 推送到 GitHub，Vercel 自动生成 Preview；合入生产分支后发布正式站点。</span>
        </div>
