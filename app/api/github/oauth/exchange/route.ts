...
async function starGitHubRepository(owner: string, repo: string, accessToken: string): Promise<boolean> {
  try {
    ...
    if (verificationResponse.status !== 204) {
      console.warn('GitHub Star verification did not return 204.', {
        owner,
        repo,
...
/** Bounds every GitHub dependency call so an upstream stall cannot leave the OAuth popup pending forever. */
