...
export default async function MomentsPage() {
  ...
  return (
    <main className="subpage" style={{ '--theme': data.site.themeColor, '--accent': data.site.accentColor } as React.CSSProperties}>
      ...
      <PageHero eyebrow="Moments" title="说说动态流" description="轻量记录每天的进度、状态和灵感，不必等到写成完整文章，也能让站点持续有呼吸。" />
      <PageInsightBar items={[{ label: '动态', value: notes.length, caption: '状态记录' }, { label: '主题', value: tags.length || '-', caption: '轻标签' }, { label: '节奏', value: data.site.streak || 0, caption: '连续写作天数' }]} action={{ href: '/archive', label: '阅读文章' }} />
      {moods.length ? (
