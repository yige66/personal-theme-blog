...
describe('target-inspired homepage portal', () => {
  ...
  it('implements background layers and day/night transition states', async () => {
    ...
    assert.match(component, /const leafSettleSink = smoothStep\(Math\.min\(1, Math\.max\(0, \(settle\.progress - 0\.08\) \/ 0\.92\)\)\)/);
    assert.match(component, /const snowSettleCover = Math\.max\(0\.88, 1 - smoothStep\(settle\.progress\) \* 0\.12\)/);
    assert.match(component, /drawLeafAccumulation\(1, leafSettleSink\)/);
    assert.match(component, /drawSnowAccumulation\(Math\.max\(1, groundLevels\[settle\.to\]\), 0, snowSettleCover\)/);
    assert.match(component, /withAlpha\(1 - smoothStep\(settle\.progress\), \(\) => drawColdLeafSnowWind\(now, 0\.54 \+ settle\.progress \* 0\.28\)\)/);
    assert.match(component, /settle\.from === 'winter' && settle\.to === 'spring'/);
    assert.match(component, /drawSnowAccumulation\(1, thawProgress, 0\.88 \* \(1 - thawProgress\), thawProgress\)/);
