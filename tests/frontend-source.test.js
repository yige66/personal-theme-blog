...
describe('target-inspired homepage portal', () => {
  ...
  it('implements background layers and day/night transition states', async () => {
    ...
    assert.match(component, /Math\.round\(intensity \/ 5\.2\)/);
    ...
    assert.match(component, /const drawLeafAccumulation[\s\S]*index < 76/);
    assert.match(component, /for \(let index = 0; index < 128; index \+= 1\)/);
    ...
    assert.match(component, /const snowSettleCover = 1 - smoothStep\(settle\.progress\) \* 0\.55/);
