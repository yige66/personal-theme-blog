...
describe('target-inspired homepage portal', () => {
  ...
  it('implements background layers and day/night transition states', async () => {
    ...
    assert.match(component, /const drawLeafAccumulation[\s\S]*index < 46/);
    assert.match(component, /for \(let index = 0; index < 88; index \+= 1\)/);
    ...
    assert.match(homeCss, /html body:has\(\.xh-clean-home\) \.xh-season-switch[\s\S]*width: 132px !important/);
