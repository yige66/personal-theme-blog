...
describe('target-inspired homepage portal', () => {
  ...
  it('implements background layers and day/night transition states', async () => {
    ...
    assert.match(component, /index < 820/);
    assert.match(component, /index < 320/);
    ...
    assert.match(component, /const preservedCurrentGround = currentSeason === 'autumn' && target === 'winter'[\s\S]*seasonGroundLevelsRef\.current\[currentSeason\] = Math\.max\(seasonGroundLevelsRef\.current\[currentSeason\], preservedCurrentGround\)/);
    ...
    assert.match(homeCss, /html body \.xh-theme-switch small,[\s\S]*html body \.xh-season-switch small[\s\S]*font-size: 9\.25px !important/);
    assert.match(homeCss, /html body \.xh-theme-switch,[\s\S]*html body \.xh-season-switch,[\s\S]*width: 140px !important/);
