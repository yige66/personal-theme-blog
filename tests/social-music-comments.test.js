...
describe('target-style music, friends, and GitHub comments', () => {
  ...
  it('adds target-style Gitalk comment surfaces and a secure GitHub OAuth proxy', async () => {
    ...
    assert.match(friendsClient, /const siteName = \[site\.title, site\.brandSuffix\]/);
    ...
    assert.match(homeCss, /html body:has\(:is\(\.subpage, \.article-page, \.friends-page\)\) :is\(\.github-comments-shell, \.github-comments-card\.is-compact, \.friends-comments, \.moment-comments-shell\)[\s\S]*background: transparent !important/);
    assert.match(homeCss, /html body:has\(:is\(\.subpage, \.article-page, \.friends-page\)\) :is\(\.github-comments-shell, \.github-comments-card\.is-compact, \.friends-comments, \.moment-comments-shell\)[\s\S]*width: min\(1240px, calc\(100% - 42px\)\) !important[\s\S]*max-width: 1240px !important/);
    ...
    assert.match(homeCss, /html body:has\(:is\(\.subpage, \.article-page, \.friends-page\)\) :is\(\.github-comments-shell, \.github-comments-card, \.friends-comments, \.moment-comments-shell\) :is\(\.custom-gitalk-glass, \.moment-gitalk\) \.gt-comment \{[\s\S]*transform: none !important[\s\S]*opacity: 1 !important/);
